...
for iter = 1:1:numel(unique(max_idx))
    % hyp(iter) = []; hyp2(iter) = [];
    ...
    hyp2(iter) = minimize(hyp2(iter), @gp, -100, @infExact, [], covfunc{iter}, likfunc{iter}, y(max_idx(3:end)==iter), y(max_idx(2:end-1)==iter));
    exp(hyp2(iter).lik);
    nlml2(iter) = gp(hyp2(iter), @infExact, [], covfunc{iter}, likfunc{iter}, y(max_idx(2:end)==iter), y(max_idx(1:end-1)==iter));
end
clc;
...
for iter = 1:1:length(ys)
    if iter == 1
        ...
        y_data = y(max_test_idx==model_idx);
        % y = [ys(iter-1);y];
