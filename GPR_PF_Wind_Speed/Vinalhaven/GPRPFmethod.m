...
x = Xtest1(1,:); % initial actual state
[row,column]=size(x);
...
for t = 1:T
    ...
    x = [GPR1step(x, Xtrain1, Ytrain) x(1) x(2)] +  sqrt(x_N)*randn(1,3);
    z = Ytest(t);
...
%         x_P_update(i) = 0.5*x_P(i) + 25*x_P(i)/(1 + x_P(i)^2) + 8*cos(1.2*(t-1)) + sqrt(x_N)*randn;
        x_P_update(i,:)=[GPR1step(x_P(i,:), Xtrain1, Ytrain) x_P(i,1) x_P(i,2)] +  sqrt(x_N)*randn(1,3);
...
%%
Yhat1GPRPFVinalhaven=x_est_out(:,1);

[R21,RMSE1] = rsquare(Ytest,Yhat1GPRPFVinalhaven);
MAPE1 = mean(abs(Yhat1GPRPFVinalhaven-Ytest)./Ytest);

figure;
plot(Ytest, 'b' ,'linewidth',3);
hold on;
plot(Yhat1GPRPFVinalhaven, '-.r','linewidth',3);
